...
public:
    ...
    RobustMatcher() : ratio(0.65f), refineF(true), confidence(0.99), distance(3.0)
    {
        ...
        Ptr<DescriptorExtractor> extractor = SIFT::create(minHessian);
    }
    
    // set the feature detector
    void setFeatureDetector( Ptr<Feature2D>& detect )
    {
        detector = detect;
    }
    
    // set the descriptor extractor
    void setDescriptorExtractor( Ptr<SIFT>& desc )
    {
        extractor = desc;
    }
                   ...
                   vector<DMatch>& outMatches)
    {
        ...
        if (refineF)
        {
            ...
            fundamental = findFundamentalMat( Mat(points1), Mat(points2), FM_8POINT );
